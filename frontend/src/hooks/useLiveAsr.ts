...
export interface UseLiveAsrResult {
  ...
  stop: () => void;
  /** One-shot transcription of a ready Float32 clip (e.g. an uploaded file). */
 ...
 * up. Built on {@link useAsr}, so model loading/backends are shared.
 */
export function useLiveAsr(model: string = DEFAULT_ASR_MODEL): UseLiveAsrResult {
  ...
  // Decode the take-so-far and transcribe its tail window. Silent on transient
  ...
  const runWindow = useCallback(async () => {
    ...
    try {
      ...
      const windowed =
        full.length > maxSamples ? full.slice(full.length - maxSamples) : full;
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      rec.onstop = () => void runWindow(); // final transcription of the full take
  ...
  const transcribeClip = useCallback(
    async (audio: Float32Array, args?: AsrRunArgs) => {
      setError(null);
      try {
        const res = await transcribe(audio, args);
        setText(res.text);
      } catch (e) {
        setError(e instanceof Error ? e.message : String(e));
      }
