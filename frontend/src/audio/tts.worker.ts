// Dedicated Web Worker for the **text → speech** modality. Two engines behind
// the uniform `TtsSynthesizer` interface: kokoro-js (Kokoro-82M) and the
// Transformers.js `text-to-speech` pipeline (MMS-VITS, SpeechT5 with speaker
// embeddings). Kept separate from the generic pipeline worker so kokoro-js
// lands in its own bundle.
//
// It used to carry MusicGen as a third engine for `/text-to-audio`. That route
// is gone — MusicGen-small is 599 MB on WASM and 1127 MB on WebGPU, past the
// ~1 GB ceiling `adding-a-task-page.md` §0 sets, and it was the only checkpoint
// the task had. The `TtsSynthesizer` seam it proved is still the right shape if
// a browser-sized music model ever appears.
