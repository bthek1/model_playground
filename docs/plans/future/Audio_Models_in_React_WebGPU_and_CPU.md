...
> to use, and — for the five tasks already built — where the code lives. No Python
> server in the inference path.
>
> This is the reference file for the other guides in this folder. Sections 1 and 2
> (backend selection, audio I/O) are the shared plumbing every category cites.

**Audio is the category Model Playground finished first.** Five of the six tasks in
the taxonomy have working routes, so most of this file is a map of shipped code
rather than a proposal. Read it that way: where a section says *shipped*, the design
...
| Voice Activity Detection | `/tasks/voice-activity-detection` | **Not built** — see §3.6 |

The build-out is recorded in
[`../completed/audio-models-in-browser.md`](../completed/audio-models-in-browser.md)
and [`../completed/audio-to-audio-deepfilternet.md`](../completed/audio-to-audio-deepfilternet.md).
...
- **`onnxruntime-web`** — bare ONNX, for the one task with no pipeline (§3.5).
...
**One worker per modality, not per task.** The audio category has four, and the
split is deliberate:
...
(`asrEngine`/`pipelineEngine`/`ttsEngine`/`enhanceEngine`) that imports nothing from
...
### 3.6 Voice Activity Detection — not built

The one gap in the category, and the cheapest page left in it. Currently falls
through to the `/tasks/voice-activity-detection` placeholder.

| Model | Notes |
|---|---|
| `onnx-community/silero-vad` | the standard. ~2 MB, streaming, frame-level speech probability |
| WebRTC-style energy VAD | no model at all; a useful "does this need a network?" baseline to ship alongside |

VAD is a good page because the output is a *timeline*, not a label: draw the speech
probability under the waveform and let the user drag a threshold. It also has an
obvious second life as a gate in front of ASR — `useLiveAsr` currently transcribes
silence as eagerly as speech.

The shape is settled: bare ONNX with a small hand-written frame loop, so
[`audio/enhance/`](../../../frontend/src/audio/enhance/) is the precedent to copy,
not `pipeline.worker.ts`. Silero-VAD carries recurrent state across frames, which is
the one genuinely new thing — the session must hold `h`/`c` between calls and reset
them per take.
...
[`Multimodal_Models_in_React_WebGPU_and_CPU.md`](Multimodal_Models_in_React_WebGPU_and_CPU.md)
§3.5.
...
| **Voice Activity Detection** | Yes — **not built** | `onnx-community/silero-vad` | WASM (2 MB) | — |
...
- **The page contract**: [`../../standards/model-page-pattern.md`](../../standards/model-page-pattern.md).
- **Recipes**: [`../../guides/adding-a-model.md`](../../guides/adding-a-model.md) §8
  (Transformers.js) and §9 (a bare ONNX graph).
- **What shipped**: [`../completed/audio-models-in-browser.md`](../completed/audio-models-in-browser.md),
  [`../completed/audio-to-audio-deepfilternet.md`](../completed/audio-to-audio-deepfilternet.md).
